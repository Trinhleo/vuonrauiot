 ...
 */
 exports.read = function(req, res) {
  ...
  dbmanage.isCurrentUserOwner = req.user && dbmanage.user && dbmanage.user._id.toString() === req.user._id.toString() ? true : false;
 ...
 */
 exports.dbmanageByID = function(req, res, next, id) {
  ...
  Dbgardens.findById(id).populate('user', 'displayName').exec(function (err, dbmanage) {
    if (err) {
      return next(err);
    } else if (!dbmanage) {
